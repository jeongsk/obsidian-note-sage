...
export const ko: TranslationKeys = {
	...
	settings: {
		...
		skills: {
			...
			instructionsDesc: 'Claude가 이 Skill을 실행할 때 따라야 하는 지침',
			instructionsPlaceholder: 'Skill의 동작 방식과 수행해야 할 작업을 설명하세요...',
			examplesLabel: 'Examples',
			examplesDesc: '이 Skill의 사용 예시를 보여주는 예제',
			examplesPlaceholder: '사용자 입력 예시와 예상 출력을 작성하세요...',
			preview: '미리보기',
			...
			deleteTitle: 'Skill 삭제',
			deleteConfirmMessage: '{name}을(를) 삭제하시겠습니까?',
			deleteUndoHint: '10초 이내에 이 작업을 취소할 수 있습니다.',
			deleteSuccess: 'Skill "{name}"이(가) 삭제되었습니다.',
			deleteError: 'Skill 삭제 실패: {error}',
			undo: '실행 취소',
			restoreSuccess: 'Skill이 복원되었습니다.',
			restoreError: 'Skill 복원에 실패했습니다.',
			// AI Validation
