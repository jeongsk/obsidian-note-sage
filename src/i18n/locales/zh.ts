...
export const zh: TranslationKeys = {
	...
	settings: {
		...
		skills: {
			...
			instructionsDesc: 'Claude 执行此 Skill 时应遵循的指令',
			instructionsPlaceholder: '描述此 Skill 应如何工作以及要执行的任务...',
			examplesLabel: '示例',
			examplesDesc: '演示此 Skill 的使用示例',
			examplesPlaceholder: '编写示例输入和预期输出...',
			preview: '预览',
			...
			deleteUndoHint: '您可以在10秒内撤销此操作。',
			deleteSuccess: 'Skill "{name}" 已删除。',
			deleteError: '删除 Skill 失败: {error}',
			undo: '撤销',
			restoreSuccess: 'Skill 已恢复。',
			restoreError: 'Skill 恢复失败。',
			// AI Validation
